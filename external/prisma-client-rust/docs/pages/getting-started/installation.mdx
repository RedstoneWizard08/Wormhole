...
<Callout type="info">
    If using Apple Silicon it is recommended to ensure Rosetta is installed,
    **even if you have installed it previously**. Run `softwareupdate
    --install-rosetta --agree-to-license` to install it. Not doing so may result
    in nondescript errors from Prisma's `query-engine` binary.
</Callout>
