...
pub struct ZeroArgAsyncMarker;

impl<TCtx, TCtxFut, TFunc, TState> TCtxFunc<TCtx, TState, ZeroArgAsyncMarker> for TFunc
where
    TCtxFut: Future<Output = TCtx>,
    TFunc: Fn() -> TCtxFut + Clone + Send + Sync + 'static,
    TState: Send + Sync,
    TCtx: Send + 'static,
{
    async fn exec(&self, _: Parts, _: &TState) -> Result<TCtx, ExecError> {
        Ok(tokio::runtime::Handle::current().block_on(self.clone()()))
    }
}
