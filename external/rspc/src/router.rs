use std::{
    collections::BTreeMap, fs::{self, File}, io::Write, marker::PhantomData, path::{Path, PathBuf}, pin::Pin, sync::Arc
};

use futures::Stream;
use serde_json::Value;
use specta::{
    ts::{self, datatype, export_named_datatype, ExportConfig},
    DataType, TypeMap,
};
...
export type Procedures = {{
    ...
    subscriptions: {subscriptions_ts}
        ...
        for export in self
            ...
            .filter(|v| v.1.name() != "()")
            .map(|v| v.1)
            .map(|v| export_named_datatype(&config, v, &mut map).unwrap())
        {
            writeln!(file, "\n{}", export)?;
        }
...
fn generate_procedures_ts<Ctx>(
    ...
    match procedures.len() {
        ...
        _ => procedures
            .iter()
            .map(|(key, operation)| {
                let input = match &operation.ty.arg_ty.inner {
                    DataType::Tuple(def)
                    ...
                    #[allow(clippy::unwrap_used)] // TODO
                    ty => datatype(config, ty, &mut map).unwrap(),
                };
                #[allow(clippy::unwrap_used)] // TODO
                let result_ts = datatype(config, &operation.ty.result_ty.inner, &mut map).unwrap();
